import type { Event, PastEvent, Sponsor, NavLink } from './types'

// ---------------------------------------------------------------------------
// Navigation
// ---------------------------------------------------------------------------
export const NAV_LINKS: NavLink[] = [
  { label: 'Events', href: '#events' },
  { label: 'Lineup', href: '#lineup' },
  { label: 'Tickets', href: '#tickets' },
  { label: 'Experience', href: '#experience' },
  { label: 'Contact', href: '#contact' },
]

// ---------------------------------------------------------------------------
// Upcoming Event
// Replace image URLs with your own once you have assets.
// For real deployment: pull this from Sanity CMS.
// ---------------------------------------------------------------------------
export const UPCOMING_EVENT: Event = {
  id: 'evt-001',
  slug: 'midsummer-nights-houston-2025',
  name: 'Midsummer Nights',
  tagline: 'An Evening of Soul & Desire',
  date: '2025-08-16T22:00:00-05:00',
  doorsOpen: '9:00 PM',
  showTime: '10:00 PM',
  venue: 'White Oak Music Hall',
  address: '2915 N Main St',
  city: 'Houston',
  state: 'TX',
  // Swap with your hero image — high-res, portrait works best
  heroImage: '/images/hero-placeholder.jpg',
  description:
    'One night. Curated sounds. The R&B experience Houston has been waiting for. LoveNRnb returns with its most intimate show yet — live performances, exclusive VIP access, and an atmosphere built for the culture.',
  ageRestriction: '21+',
  isUpcoming: true,
  isSoldOut: false,
  lineup: [
    {
      id: 'artist-001',
      name: 'VanJess',
      role: 'headliner',
      image: '/images/artists/vanjess.jpg',
      genre: 'R&B / Soul',
      instagram: 'https://instagram.com/vanjessmusic',
      spotify: 'https://open.spotify.com/artist/1hBs9UPdXcNaKYkiqbHLsY',
    },
    {
      id: 'artist-002',
      name: 'Terrace Martin',
      role: 'featured',
      image: '/images/artists/terrace-martin.jpg',
      genre: 'Jazz / R&B',
      instagram: 'https://instagram.com/terracemartin',
    },
    {
      id: 'artist-003',
      name: 'Amara La Negra',
      role: 'supporting',
      image: '/images/artists/amara.jpg',
      genre: 'Afro-Latin R&B',
      instagram: 'https://instagram.com/amaralanegra',
    },
    {
      id: 'artist-004',
      name: 'Masego',
      role: 'supporting',
      image: '/images/artists/masego.jpg',
      genre: 'TrapHouseJazz',
      instagram: 'https://instagram.com/masego',
    },
    {
      id: 'artist-005',
      name: 'TBA',
      role: 'opener',
      image: '',
      genre: 'R&B',
      tba: true,
    },
    {
      id: 'artist-006',
      name: 'TBA',
      role: 'opener',
      image: '',
      genre: 'R&B',
      tba: true,
    },
  ],
  ticketTiers: [
    {
      id: 'tier-ga',
      name: 'General Admission',
      price: 45,
      perks: [
        'General floor access',
        'Cash bar access',
        'Live performances',
        'Photo opportunities',
      ],
      available: 312,
      total: 500,
      isHighlighted: false,
      ctaLabel: 'Get GA Tickets',
      ticketUrl: 'https://tixr.com',
    },
    {
      id: 'tier-vip',
      name: 'VIP Access',
      price: 95,
      perks: [
        'Priority entry — skip the line',
        'Dedicated VIP section',
        '2-hour premium open bar',
        'Complimentary champagne on arrival',
        'Exclusive artist access lounge',
      ],
      available: 48,
      total: 100,
      isHighlighted: true,
      ctaLabel: 'Secure VIP',
      ticketUrl: 'https://tixr.com',
    },
    {
      id: 'tier-table',
      name: 'Table Service',
      price: 250,
      priceLabel: 'from $250 / seat',
      perks: [
        'Premium reserved table',
        'Dedicated bottle service host',
        'First-pour bottle package included',
        'VIP lounge access',
        'Complimentary guest list (2 add-ons)',
      ],
      available: 8,
      total: 12,
      isHighlighted: false,
      ctaLabel: 'Reserve a Table',
      ticketUrl: 'https://tixr.com',
    },
  ],
}

// ---------------------------------------------------------------------------
// Past Events
// ---------------------------------------------------------------------------
export const PAST_EVENTS: PastEvent[] = [
  {
    id: 'past-001',
    name: "Valentine's Affair",
    date: '2025-02-14',
    venue: 'The Ballroom at Bayou Place',
    city: 'Houston',
    attendees: 420,
    coverImage: '/images/past/valentines-cover.jpg',
    images: [
      '/images/past/valentines-1.jpg',
      '/images/past/valentines-2.jpg',
      '/images/past/valentines-3.jpg',
      '/images/past/valentines-4.jpg',
    ],
    recapVideoUrl: '',
  },
  {
    id: 'past-002',
    name: 'Autumn Serenade',
    date: '2024-10-19',
    venue: 'Revention Music Center',
    city: 'Houston',
    attendees: 380,
    coverImage: '/images/past/autumn-cover.jpg',
    images: [
      '/images/past/autumn-1.jpg',
      '/images/past/autumn-2.jpg',
      '/images/past/autumn-3.jpg',
    ],
  },
  {
    id: 'past-003',
    name: 'Summer Solstice',
    date: '2024-06-21',
    venue: 'House of Blues Houston',
    city: 'Houston',
    attendees: 350,
    coverImage: '/images/past/summer-cover.jpg',
    images: [
      '/images/past/summer-1.jpg',
      '/images/past/summer-2.jpg',
    ],
  },
]

// ---------------------------------------------------------------------------
// Sponsors
// ---------------------------------------------------------------------------
export const SPONSORS: Sponsor[] = [
  {
    id: 'spon-001',
    name: 'Hennessy',
    logo: '/images/sponsors/hennessy.svg',
    tier: 'presenting',
    url: 'https://www.hennessy.com',
  },
  {
    id: 'spon-002',
    name: 'ESSENCE',
    logo: '/images/sponsors/essence.svg',
    tier: 'partner',
    url: 'https://www.essence.com',
  },
  {
    id: 'spon-003',
    name: 'Spotify',
    logo: '/images/sponsors/spotify.svg',
    tier: 'partner',
    url: 'https://www.spotify.com',
  },
  {
    id: 'spon-004',
    name: 'Fashion Nova',
    logo: '/images/sponsors/fashionnova.svg',
    tier: 'community',
    url: 'https://www.fashionnova.com',
  },
  {
    id: 'spon-005',
    name: 'House of CB',
    logo: '/images/sponsors/houseofcb.svg',
    tier: 'community',
    url: 'https://www.houseofcb.com',
  },
]
